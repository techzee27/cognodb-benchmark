"""
Load the dataset into Neo4j AuraDB Free via the official Neo4j driver.

Usage:
    python -m loaders.load_neo4j --nodes data/nodes.csv --edges data/edges.csv
"""
...
def load(nodes_path: str, edges_path: str) -> dict[str, float | int]:
    """Load nodes and edges CSV files into Neo4j AuraDB Free in batches.
    ...
    print(f"Neo4j load complete: {node_count} nodes, {rel_count} relationships")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load graph data into Neo4j AuraDB Free")
